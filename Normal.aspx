<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="~/Site1.Master" CodeBehind="Normal.aspx.vb"
    Inherits="LeaveSoftware.Normal" Title="الاجازات الاعتيادية" %>
  ...
  <td dir="rtl">
       ...
       <td>
         ...
         <asp:Panel ID="Panel1" runat="server" GroupingText="بحث رقم الباج" 
             ...
             <asp:RegularExpressionValidator ID="RegularExpressionValidator2" runat="server" ErrorMessage="رقم صحيح فقط" ValidationExpression="^\d+$" ControlToValidate="TextBoxBatch" Display="Dynamic"></asp:RegularExpressionValidator>
     ...
     style="height: 550px; width: 903px;" Visible="False" >
        ...
        InsertCommand="INSERT INTO LeaveInfo(Date, Duration, LeaveTypeID, EmployeeID, Notes, OfficialOrderNumber) VALUES (@Date, @Duration, 1, @EmployeeID, @Notes, @OfficialOrderNumber)">
            ...
            <asp:SqlDataSource ID="SDSBalance" runat="server" 
                ...
                SelectCommand="SELECT LeaveBalance.Balance, LeaveBalance.ActualLeaves, LeaveBalance.RemainingLeaves, LeaveBalance.LeaveTypeID, Employee.BatchNumber FROM LeaveBalance INNER JOIN LeaveType ON LeaveBalance.LeaveTypeID = LeaveType.ID INNER JOIN Employee ON LeaveBalance.EmployeeID = Employee.ID WHERE (LeaveBalance.LeaveTypeID = 1) AND (Employee.BatchNumber = @BatchNumber)">
                <SelectParameters>
                    <asp:ControlParameter ControlID="TextBoxBatch" Name="BatchNumber" 
                        PropertyName="Text" />
                </SelectParameters>
