<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="~/Site1.Master" CodeBehind="Sick.aspx.vb"
    Inherits="LeaveSoftware.Sick" Title="Untitled Page" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="CPHContent" runat="server">
    <h2>
        صفحة الاجازات المرضية</h2>     
        ...
        <asp:DetailsView ID="DtlViewEmployeeInfo" runat="server" Height="72px" 
     ...
     style="height: 550px; width: 903px;" Visible="False" >
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" 
            ...
            SelectCommand="SELECT Employee.Name AS [الاسـم], Employee.BatchNumber AS [رقم الباج], Employee.StartDate AS [تاريخ المباشرة], Department.Name AS [الدائرة/القسم], Employee.QuitDate AS [تاريخ ترك الوظيفة], Employee.EmployeeStatus AS [حالة الموظف], Employee.ID FROM Employee INNER JOIN Department ON Employee.DepartmentID = Department.ID WHERE (Employee.BatchNumber = @BatchNumber)">
                <SelectParameters>
                    <asp:ControlParameter ControlID="TextBoxBatch" Name="BatchNumber" 
                        PropertyName="Text" />
                </SelectParameters>
            </asp:SqlDataSource>
        
            <asp:SqlDataSource ID="SDSBalance" runat="server" 
                ConnectionString="<%$ ConnectionStrings:Leave_DBConnectionString %>" 
                
                
                
                
        SelectCommand="SELECT LeaveBalance.Balance, LeaveBalance.ActualLeaves, LeaveBalance.RemainingLeaves, LeaveBalance.LeaveTypeID, Employee.BatchNumber FROM LeaveBalance INNER JOIN LeaveType ON LeaveBalance.LeaveTypeID = LeaveType.ID INNER JOIN Employee ON LeaveBalance.EmployeeID = Employee.ID WHERE (LeaveBalance.LeaveTypeID =3) AND (Employee.BatchNumber = @BatchNumber)">
                <SelectParameters>
                    <asp:ControlParameter ControlID="TextBoxBatch" Name="BatchNumber" 
                        PropertyName="Text" />
                </SelectParameters>
        ...
        <asp:SqlDataSource ID="SDSLeaveInfo" runat="server" 
            ConnectionString="<%$ ConnectionStrings:Leave_DBConnectionString %>" 
            
            SelectCommand="SELECT LeaveInfo.ID, LeaveInfo.Date, LeaveInfo.Duration, LeaveInfo.LeaveTypeID, LeaveInfo.Notes, LeaveInfo.OfficialOrderNumber, Employee.BatchNumber, LeaveInfo.EmployeeID FROM LeaveInfo INNER JOIN Employee ON LeaveInfo.EmployeeID = Employee.ID WHERE (LeaveInfo.LeaveTypeID = 3) AND (Employee.BatchNumber = @BatchNumber)" 
            DeleteCommand="DELETE FROM LeaveInfo WHERE (ID = @ID)" 
            InsertCommand="INSERT INTO LeaveInfo(Date, Duration, LeaveTypeID, EmployeeID, Notes, OfficialOrderNumber) VALUES (@Date, @Duration,3, @EmployeeID, @Notes, @OfficialOrderNumber)" 
            ...
            <InsertParameters>
                ...
                <asp:Parameter Name="Duration" />
                <asp:ControlParameter ControlID="DtlViewEmployeeInfo" Name="EmployeeID" 
                    PropertyName="DataKey(0)" />
                <asp:Parameter Name="Notes" />
                <asp:Parameter Name="OfficialOrderNumber" />
            </InsertParameters>
        </asp:SqlDataSource>
    
    <asp:SqlDataSource ID="SDSInsertLeaveInfo" runat="server" 
            ConnectionString="<%$ ConnectionStrings:Leave_DBConnectionString %>" 
            InsertCommand="INSERT INTO LeaveInfo(Date, Duration, LeaveTypeID, EmployeeID, Notes, OfficialOrderNumber) VALUES (@Date, @Duration, 1, @EmployeeID, @Notes, @OfficialOrderNumber)" 
            ...
            <SelectParameters>
                <asp:ControlParameter ControlID="TextBoxBatch" Name="EmployeeID" 
